...
@section('content')
    ...
    <div class="container-xxl">
        <div class="row align-items-center">
            <div class="border-0 mb-4">
                <div
                    class="card-header pb-3 no-bg bg-transparent d-flex  px-0 justify-content-between border-bottom flex-md-row flex-column align-items-md-center align-items-center ps-md-0 ps-3">
                    <!-- Left Section: Title -->
                    ...
                    <div class="form-check form-switch me-3 mt-md-0 mt-2">
                        ...
                        <input class="form-check-input" type="checkbox" id="nutritionToggle" name="nutrition_info" value="1"
                            data-payment-id="{{ $payment->id }}" {{ $firstPlan->nutrition_info_flag ? 'checked' : '' }}>
                    </div>
                                                                ...
                                                                @foreach ($columns as $columnFoods)
                                                                    <div class="col-md-6">
                                                                        @foreach ($columnFoods as $foodTitle)
                                                                            ...
                                                                            <div class="form-check" id="food-wrapper-{{ $foodId }}"
                                                                                data-category-id="{{ $subQuestion }}">
                                                                                <input type="checkbox" name="setp5_foods[]"
                                                                                    value="{{ $foodId ?? '' }}"
                                                                                    class="form-check-input food-checkbox"
                                                                                    id="setp5Food{{ $foodId ?? md5($foodTitle) }}"
                                                                                    data-food-id="{{ $foodId ?? '' }}"
                                                                                    data-food-name="{{ $foodTitle }}" {{ $foodId ? '' : 'disabled title="Food not found"' }}>
                                                                                <label class="form-check-label food-label"
                                                                                    for="setp5Food{{ $foodId ?? md5($foodTitle) }}">
                                                                                    {{ $foodTitle }}
                                                                                </label>
                                                                            </div>
                                                                        @endforeach
                                                                ...
                                                                @foreach ($columns as $columnFoods)
                                                                    <div class="col-md-6">
                                                                        @foreach ($columnFoods as $foodTitle)
                                                                            ...
                                                                            @if ($foodId)
                                                                                <div class="form-check" id="food-wrapper-{{ $foodId }}"
                                                                                    data-category-id="{{ $counter }}">
                                                                                    <input type="checkbox" name="setp5_foods[]" value="{{ $foodId }}"
                                                                                        class="form-check-input food-checkbox"
                                                                                        id="setp5Food{{ $foodId }}" data-food-id="{{ $foodId }}"
                                                                                        data-food-name="{{ $foodTitle }}">
                                                                                    <label class="form-check-label food-label"
                                                                                        for="setp5Food{{ $foodId }}">
                                                                                        {{ $foodTitle }}
                                                                                    </label>
                                                                                </div>
                                                                                @php $counter++; @endphp
    ...
    <div class="modal" id="savePlanModal" tabindex="-1" aria-labelledby="savePlanModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" id="leaveWithoutSaving" data-bs-dismiss="modal">No,
                        Leave</button>
                ...
                <div class="modal-body">
                    <form id="editSwapItemForm">
                        ...
                        <div class="mb-3 mt-3 mx-3">
                            ...
                            <p>Energy: <span id="modalEnergy">0kJ</span> | Protein: <span id="modalProtein">0g</span> |
                                Carb: <span id="modalCarbs">0g</span> | Fat: <span id="modalFat">0g</span></p>
                        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <button type="button" class="btn btn-primary mb-2 add-food-button" id="searchWoolworthFood"
                        data-plan-id="" data-meal-id="" data-meal-time-id="" data-user-id="">
                        Add Woolworths Food
                    </button>
                </div>
    ...
    <div class="modal" style="display:none;" id="mealFoodAddModal" tabindex="-1" aria-labelledby="mealFoodAddModal"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="mealFoodAddModalLabel">Select Swap Foods</h5>
                    <button type="button" class="btn-close" id="closeMealFoodAddModal" data-bs-dismiss="modal"
                            ...
                            tableBody.append(`
                                <tr id="itemRow_${planId}_${mealTimeId}_${mealId}_${food.id}" data-item-id="${item.id}">
                                    <td class="text-wrap" width="50%">
                                        ...
                                        <div class="row">
                                            <div class="col">
                                                        <p class="px-2 mb-2 fw-bold">(${typeof food.qty === 'string' && food.qty.includes('/')
                                    ? food.qty
                                    : (
