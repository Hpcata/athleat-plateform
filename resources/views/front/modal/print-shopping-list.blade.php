<!-- Shopping List Modal -->
<div id="print-shopping-list-modal">
    <div class="shopping-list-wrap">
        <div style="padding: 16px; border-bottom: 1px solid #d8d8d8; display: flex; align-items: center; justify-content: space-between;">
            <span style="font-size: 18px; font-weight: 700; color: #080808;">Print Shopping list</span>
            <button id="print-shopping-list-close" aria-label="Close" style="background: none; border: none; font-size: 1.6rem; color: #222; cursor: pointer;">&times;</button>
        </div>
        <div style="flex: 1 1 auto; overflow-y: auto; padding: 18px 24px 24px 24px; max-height: 70vh;">
            <div id="shopping-list-content" style="font-size: 1rem; color: #222;"></div>
        </div>
        <div style="text-align: end; padding: 12px 16px; border-top: 1px solid #d8d8d8;display: flex;justify-content: end;" class="responsive-modal-footer">
            <button id="download-pdf" class="btn btn-primary" style="padding: 12px 28px; font-size: 1rem; font-weight: 500; cursor: pointer; box-shadow: 0 2px 8px rgba(42, 92, 164, 0.08);">Download PDF</button>
        </div>
    </div>
</div>

